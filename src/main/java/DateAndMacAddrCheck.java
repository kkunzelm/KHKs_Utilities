...
public class DateAndMacAddrCheck {
	...
	public static void main(String args[]) {
		...
		if (args.length == 1) {
			...
			if (args[0].equals("-help") || args[0].equals("--help") || args[0].equals("/?")) {
				...
				manuallySetExpirationDate = args[0]; // overwrites the expirationDate with the first commandline
														// argument
			...
			// man könnte auch einen String als 2. Argument verwenden und dann in die Bytes
			// zerlegen.
	...
	public static boolean mainOfKHKsCopyProctection() {
		 ...
		 * System.out.println("Heureka ..."); // Code for plugin execution } else {
		 * 
		...
		if (expirDataFileExists == false) {
			try {
				...
				// wenn das File neu angelegt wird, darf man auf jeden Fall schon mal
				// weiterarbeiten
		...
		if (expirDataFileExists == true) {
            ...
            // ist die Kurzschreibweise für expirCheck == true
            // ist eigentlich überflüssig, da Vorgabe
            allowUseOfPlugin = expirCheck();
		}
	...
	private static boolean readExpirDataFile() throws IOException {
		...
		// siehe hier:
		// http://www.java2s.com/Code/Java/File-Input-Output/newDataInputStreamnewBufferedInputStreamnewFileInputStream.htm
		String expirDataPathAndFile = null;
		if (Prefs.getHomeDir() == null) {

			// Einschub:
			// Wenn ich das File von der Console öffne, ohne classpath zu ImageJ, dann geht
			// Prefs.getHomeDir nicht
			// Als Workaround wähle ich ein anderes Verzeichnis
			// Mit System.getProperty("String") kann man verschiedene Werte von
			...
			expirDataPathAndFile = System.getProperty("user.home") + System.getProperty("file.separator")
					+ "KHKdata.txt";
		} else {
			expirDataPathAndFile = Prefs.getHomeDir() + Prefs.getFileSeparator() + "KHKdata.txt"; // richtigen Path
																									// separator
																									// verwenden
		}

		if (debug)
			System.out.println("DataFile inkl. Pfad: " + expirDataPathAndFile);
		...
		try {
			...
			// if (debug) System.out.println("readExpirDataFile: MacAddress:
			// "+macAddressFromExpirDataFile);
			if (debug) {
				System.out.println("readExpirDataFile: ExpirDate: " + expirDateFromExpirDataFile + " MacAdr.: "
						+ macAddressFromExpirDataFile);
			}
	...
	private static void generateNewExpirDataFile() throws IOException {
		...
		// siehe:
		// http://www.java2s.com/Code/Java/File-Input-Output/newDataOutputStreamnewBufferedOutputStreamnewFileOutputStream.htm
		...
		if (debug)
			System.out.println("Datum..Formatter: " + formatter.format(currentDateAndTime.getTime()));
		...
		if (debug) {
			...
			/*
			 * Extract a array of mac address and convert it to hexa with the following
			...
			for (int i = 0; i < b1.length; i++) {
				...
				// %02 = 2 Stellen, X = Hexadezimal mit Grossbuchstaben, %s = String - hier mit
		...
		try {
			...
			if (debug)
				System.out.println("generateNewExpirDataFile: writeUTF: " + expirDateString);
			...
			if (manuallySetExpDateFlag) {
				...
				// TODO könnte man in eine eigene Methode auslagern, gleiches Muster 3x
				// verwendet
				...
				try {
					date = formatter.parse(manuallySetExpirationDate);
				...
				try {
					date = formatter.parse(expirDateString);
			...
			if (debug)
				System.out.println("generateNewExpirDataFile: date [days] als long: " + dateInDays);

			// macAddress as String

			for (int i = 0; i < b1.length; i++) {
				if (debug)
					System.out.println("i" + i + " <" + b1[i] + ">");
				out.writeByte(b1[i]);
			}
		} finally {
			out.close();
	...
	private static boolean compareCurrentDateWithExpirationDate() {
		...
		try {
			date = formatter.parse(expirDateFromExpirDataFile);
		...
		if (debug)
			System.out.println("temp [days]: " + temp + "\n" + "dateInDays: " + dateInDays);
		if (dateInDays != temp) {
		...
		// System.out.println("expiration date in ms: " +
		// expirationDateAndTime.getTimeInMillis());
		// System.out.println("current date in ms: " +
		// currentDateAndTime.getTimeInMillis());
		// System.out.println("aktuelle Zeit nach Exp Zeit: " +
		// currentDateAndTime.after(expirationDateAndTime));
	...
	private static boolean expirCheck() {
		...
		if (!valideExpirData) {
			...
			System.out.println("Send an email with the name of the plugin and this information to the author: "
					+ getFormatedMacAdressEth0());

			if (valideMacAddress)
				ij.IJ.showMessage("Licence expired. Contact the author of this plugin to update the licence.\n"
						+ "Send an email with the name of the plugin and this information to the author:\n" + "\""
						+ getFormatedMacAdressEth0() + "\"");
			if (valideExpirDate)
				ij.IJ.showMessage("Unauthorized Copy. Contact the author of this plugin for an individual licence.\n"
						+ "Send an email with the name of the plugin and this information to the author:\n" + "\""
						+ getFormatedMacAdressEth0() + "\"");
		} else {
	...
	private static String getMacAdressEth0() {
		...
		try {
			...
			b1 = networkInterface.getHardwareAddress();
			/*
			 * if (debug) System.out.print("Hardware Address = "); for (int i = 0; i <
			 * b1.length; i++) { System.out.print(b1[i]); } if (debug)
			 * System.out.println(" ");
			 */
		...
		if (debug)
			System.out.println("getMacAdressEth0: MacAddress: " + macAddressFromEth0);
	...
	private static String getFormatedMacAdressEth0() {
		...
		macAddressFromEth0 = b1[0] + " ";
		macAddressFromEth0 += b1[1] + " ";
		macAddressFromEth0 += b1[2] + " ";
		macAddressFromEth0 += b1[3] + " ";
		macAddressFromEth0 += b1[4] + " ";
		macAddressFromEth0 += b1[5] + " ";
 ...
 * extremely easy to make a mistake and use the == operator. When we compare
 * primitive data types, such as two ints, two chars, two doubles, etc. we can
 * use the == operator. We can also use the == operator to compare two objects.
 * However, when used with an object, the == operator will only check to see if
 * they are the same objects, not if they hold the same contents. This means
 * that code like the following will not correctly compare to strings :
 * 
 * if ( string1 == string2 ) { System.out.println ("Match found"); }
 * 
 * This code will only evaluate to true if string1 and string2 are the same
 * object, not if they hold the same contents. This is an important distinction
 * to make. Checking, for example, to see if aString == "somevalue", will not
 * evaluate to true even if aString holds the same contents.
 * 
 * To correctly compare two strings, we must use the .equals method(). This
 * method is inherited from java.lang.Object, and can be used to compare any two
 * strings. Here's an example of how to correctly check a String's contents :
 * 
 * if ( string1.equals("abcdef") ) { System.out.println ("Match found"); }
 * 
 * This is a simple, and easy to remember tip that will safe you considerable
 * time debugging applications. Remember - never use the == operator if you only
 * want to compare the string's contents.
 *
 */