...
public class KHKs_AddSliceToStack implements PlugIn {
	...
	public void run(String arg) {
		...
		gd.addChoice("Position:", sPositions, sPositions[4]);
                gd.addNumericField("Slices before current stack:",beforeFirstSlice, 0,5,"pixels");
                gd.addNumericField("Slices_after current stack:",afterLastSlice, 0,5,"pixels");
		...
		int iPos = gd.getNextChoiceIndex();
                beforeFirstSlice = (int)gd.getNextNumber();
                afterLastSlice = (int)gd.getNextNumber();
...
// KHK 
                ...
                if(beforeFirstSlice > 0){		
                    for (int i=1; i<=beforeFirstSlice; i++) {
		...
		for (int i=1; i<=nFrames; i++) {
			...
			stackNew.addSlice(stackOld.getSliceLabel(i), ipNew);
                ...
                if(afterLastSlice > 0){		
                    for (int i=1; i<=afterLastSlice; i++) {
                            IJ.showProgress((double)i/nFrames);
                            ipNew = ipOld.createProcessor(wNew, hNew);
                            if (zeroFill)
                                    ipNew.setValue(0.0);
